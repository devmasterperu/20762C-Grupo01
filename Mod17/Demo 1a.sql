...
-- The value of the Phone column is 170-555-0127
	...
	SET Phone = N'170-555-0127'
	WHERE CustomerID = 2
    COMMIT TRAN
	*/
SELECT CustomerID, Phone FROM SalesLT.Customer WHERE CustomerID = 2; --2	170-555-0127
...
BEGIN TRANSACTION
	SELECT CustomerID, Phone 
	FROM SalesLT.Customer WITH (READCOMMITTEDLOCK)
	WHERE CustomerID = 2;--2	170-555-0127
...
-- Run the following query. Note that the value of the Phone column has changed during the transaction
	SELECT CustomerID, Phone 
	FROM SalesLT.Customer WITH (READCOMMITTEDLOCK)
	WHERE CustomerID = 2;--2	333-555-3333
COMMIT TRANSACTION;
