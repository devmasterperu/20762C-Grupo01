...
SELECT * FROM dbo.DatabaseLog order by DatabaseLogID desc;
GO
...
SELECT XmlEvent,
       Event_Detail.value('PostTime[1]','datetime2') AS PostTime,
       Event_Detail.value('SPID[1]', 'int') AS SPID,
       Event_Detail.value('ObjectType[1]','sysname') AS ObjectType,
       Event_Detail.value('ObjectName[1]','sysname') AS ObjectName,
	   Event_Detail.value('ServerName[1]','varchar(100)') AS ServerName
FROM dbo.DatabaseLog AS dl
CROSS APPLY dl.XmlEvent.nodes('/EVENT_INSTANCE') AS Event_Info(Event_Detail)
ORDER BY PostTime;
